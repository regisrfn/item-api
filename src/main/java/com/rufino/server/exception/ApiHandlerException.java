...
public class ApiHandlerException {
    ...
    public Map<String, String> handleSqlError(DataIntegrityViolationException e) {
        ...
        String pattern = ".*\\w*; SQL.*;.*\\[(fk_\\w+_item)\\].*";
        String error = (errorMsg.replaceAll(pattern, "$1"));
        Map<String, String> errors = new HashMap<>();

        if (error.equals("fk_order_item") || error.equals("fk_product_item")) {
            String[] errorString = error.split("_");

            if (errorString[1].equals("order")) {
                errors.put("orderId", "Order id not exists");
            } else if (errorString[1].equals("product")) {
                errors.put("orderId", "Product id not exists");
            }
        }
        return errors;
    }
